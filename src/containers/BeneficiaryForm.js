...
class BeneficiaryForm extends Component {
	...
	handleOk = () => {
		...
		saveReceiverInformation(account, nameBank, beneficiary, nameReminder)
			.then((res) => {
				this.setState({
					account: " ",
					nameBank: " ",
		...
		return (
			<div className="outletMain">
				...
				<div className=" mt-3">
					...
					>
						<Column
							title="Name Beneficiary "
							dataIndex="nameBeneficiary"
							key="nameBeneficiary"
						/>
						<Column
							title="Reminder Name "
							dataIndex="nameRemind"
							key="nameRemind"
						/>
						<Column
						...
						<Column
							title="Name Bank"
							dataIndex="linkedbank"
							key="linkedbank"
							render={(linkedbank) =>
								linkedbank && <p>{linkedbank.nameBank}</p>
							}
						/>
						<Column
							title="Action"
					...
					<Modal
						title={isLoadUpdate ? "Edit  Benefiviary " : "Add  Benefiviary "}
					...
					>
						<Form className="myForm" {...layout} form={this.form}>
							...
							{errEdit && !isEdit && <Alert message={errEdit} type="error" />}
							...
							<Form.Item label="account: " className="mt-5">
								<Input
									...
									value={account}
									onChange={(e) =>
										this.onChangeValue(e.target.value, "account")
									}
								/>
							</Form.Item>
							<Form.Item label="Bank : ">
								<Select
									onChange={(value) => this.onChangeValue(value, "nameBank")}
							...
							<Form.Item label="  Reminder: ">
								<Input
									...
									onFocus={this.onFocusEr}
									onChange={(e) =>
										this.onChangeValue(e.target.value, "nameReminder")
									}
								/>
							</Form.Item>
							<Form.Item label="  beneficiary: ">
								<Input
									...
									onFocus={this.onFocusEr}
									onChange={(e) =>
										this.onChangeValue(e.target.value, "beneficiary")
									}
								/>
							</Form.Item>
							...
							<div style={{ marginLeft: "30%" }}>
								...
								<Button
									className="ml-3"
									key="back"
									onClick={this.showModalCancelSave}
								>
									Return
								</Button>
								,
...
const mapDispatchToProps = (dispatch) => ({
	...
	saveReceiverInformation: (accountNumber, idBank, beneficiary, nameRemind) =>
		dispatch(
	...
	deleteReceiver: (idReceiver) =>
		dispatch(transactionActions.deleteReceiver(idReceiver)),
	editReceiverInformation: (
		idReceiver,
		account,
		nameBank,
		...
		dispatch(
			transactionActions.editReceiverInformation(
				idReceiver,
				account,
				nameBank,
				beneficiary,
